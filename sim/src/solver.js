...
import { outletDemandAt, outletTableMin, streamEmitterCoeff, outletThrowAt } from "./outlets.js";
...
function finalize(model, state, fx, pumpOn, valveOpen, reachable, res, topo, commandedNotOpening, iters, converged, valvesFrozen) {
  const outlets = [...model.flowNodes.values()].filter((n) => n.role === "outlet");
  const demands = new Map();
  const throws = new Map(); // rotor outletId -> throw radius (m) at its solved inlet pressure
  let outSum = 0;
  for (const o of outlets) {
    const ep = epOf(o.id);
