...
// and a multi-outlet fitting contributes its full k_minor to each edge.
...
export function buildTopology(model, state) {
  ...
  const emitters = state.emitters || new Map(); // coeff CMH/√m
  ...
  for (const L of flowNodes.values()) {
    ...
    if (L.to.length > 1) {
      // a 2-port conduit cannot branch; using to[0] would silently drop a subtree
    ...
    // partial-clog faults ride on the link as extra minor loss; full clogs seal it via closedLinks
    ...
    if (L.role === "pipe") {
      ...
      // Opening fraction t scales effective Kv to t·Kv, so the loss curve scales by 1/t² (loss ∝ (Q/Kv)²).
      // The THROTTLE_MIN clamp only keeps the curve finite; a seated screw is held shut by the solver.
