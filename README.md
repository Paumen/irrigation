...
- **Troubleshooting** — the `irrigation` skill's `troubleshoot` playbook drives a question-and-answer loop over a scoring engine (`diagnose_irrigation` MCP tool) that re-ranks the most likely root failure modes each round.
