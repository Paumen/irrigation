...
A **headless** physics engine for the system's hydraulics **and** control wiring — Node-only plain ES modules, **no UI of any kind** (no browser page, no rendering, no quasi-time mode; an earlier browser front-end was built and then removed deliberately — do not reintroduce UI or time-stepping features unless the user explicitly asks). Spec history: `docs/Sim_spec.md`, `docs/sim_implementation_plan.md`, `docs/sim_build_plan.md`.
