...
- `tools/test_diagnose.py` — questionnaire convergence regression: drives the engine fault-by-fault (following each step's top-recommended question) through a representative per-fault answer key — discrete answers plus the Q9/Q10/Q11 context matrices — and asserts the true cause reaches the top of the ranking within 15 questions, with a few documented near-degeneracies bounded in `EXPECTED_MAX`. Run with `python3 tools/test_diagnose.py`.
