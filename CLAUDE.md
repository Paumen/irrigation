...
- `docs/mockups/` — **UI interpretation partials** (not the app). Named `sim-ui-<requirement(s)>-<what-it-is>` (e.g. `sim-ui-U6-diagram`), each tied to the `sim_ui.md` requirement(s) it checks. The diagram sheets + legend come from `gen.py`; the two side-panel mocks are hand-authored. `svg2png.py` (`pip install cairosvg`) renders every SVG here to a same-named PNG — keep PNGs in sync via `python3 gen.py && python3 svg2png.py`. See `docs/mockups/README.md`.
