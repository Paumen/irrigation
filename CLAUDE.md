...
- **Status: M0–M5 done** (headless hydraulic + electrical core + static schematic, verified by `cd sim && npm install && npm test`; `npm run smoke` is the M0 EPANET spike). The schematic layer is split: `layout.js` (elkjs, once at startup) + `scene.js` (pure visual-attribute computation, harness-gated) feed `render.js` (thin SVG data-join); `index.html`/`app.js` render one fixed state via `renderState()`. Browser check: `python3 -m http.server` from repo root, open `/sim/`. M6+ (controls/live updates, quasi-time, faults) not yet built.
