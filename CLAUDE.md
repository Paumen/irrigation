...
- `docs/mockups/` — **UI interpretation partials** (not the app). Named `sim-ui-<requirement(s)>-<what-it-is>` (e.g. `sim-ui-U6-diagram`), each tied to the `sim_ui.md` requirement(s) it checks. SVG sources live in `docs/mockups/svg/`, rendered PNGs at the `docs/mockups/` root. `gen.py` generates three (`sim-ui-U6-diagram`, `sim-ui-U18-side-panel-overlay`, `sim-ui-U12-U16-diagram-line-visualizations`); the other two (`sim-ui-U8-diagram-with-panel`, `sim-ui-U18-side-panel-docked`) are hand-authored. `svg2png.py` (`pip install cairosvg`) renders every `svg/*.svg` to a same-named PNG — keep PNGs in sync via `python3 gen.py && python3 svg2png.py`. See `docs/mockups/README.md`.
