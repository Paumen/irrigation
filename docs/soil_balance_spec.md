...
- **DAT-2** Each load fetches 24 days before today + today + 15 forecast days (the Forecast API span, DAT-1); the earliest fetched days (before the shown window) act as run-up to settle the soil estimate, seeded at field capacity on the first fetched day. The graph shows a 16-day window: last 8, today, next 7.
