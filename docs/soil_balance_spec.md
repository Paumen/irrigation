...
- **UIX-6** The balance computes at full precision. In the cross-section the data values — temperature, sun/ET, rain, and watering amounts — are conveyed as glyphs, not numbers (per UIX-2/UIX-3). Numeric text elsewhere is fine: the axis day labels, the control inputs (e.g. watering duration, CTR-1), and today's status readout — fullness as an integer %, and water held / capacity as integer mm, the latter display-rounded only.
- **UIX-7** Status verdict — one headline drives action (SCO-1), set by the LOG-5 thresholds and evaluated in order (first match wins): (1) at/below the empty threshold → *Water today*; (2) otherwise below the refill point → *Water now*; (3) otherwise (above the refill point) → *Water tomorrow* (one day until the projected level first drops below the refill point) or *Water in N days* (N such days, capped at the window end with a `+`). Tone escalates calm → warn → urgent as the verdict sharpens (≥2 days = calm, tomorrow = warn, now/today = urgent). The headline pairs with the fullness % and the held / capacity mm readout (UIX-6).
