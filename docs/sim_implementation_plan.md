# Irrigation System Simulator — Implementation Plan (Build Spec)
...
- Valve and fitting resistance — the automatic valve's loss from the `valve.auto_loss` table in
...
- **Valve won't open** — an automatic valve needs at least its `min_bar` (1.5 bar) at the inlet
...
- For display, mark each wire **energised** or not — `solveElectrical` returns the `energisedWires` set
  (wires carrying current on a closed path); a single broken wire reads dead while its neighbours stay lit.
...
- The wiring shows which runs are energised (`energisedWires`), each wire drawn individually.
