...
const BOXES = [
  {key:'sw',     x:  36, y: 20, w: BOX_W, h: BOX_H, label:'SOFTWARE',   icon:'sw'},
  {key:'ctrl',   x: 202, y: 20, w: BOX_W, h: BOX_H, label:'CONTROLLER', icon:'ctrl'},
  {key:'relay',  x: 368, y: 20, w: BOX_W, h: BOX_H, label:'RELAY',      icon:'relay'},
  {key:'pump',   x: 534, y: 20, w: BOX_W, h: BOX_H, label:'PUMP',       icon:'pump'},
  {key:'valves', x: 534, y:170, w: BOX_W, h: BOX_H, label:'VALVES',     icon:'valves'},
];
const SPRK = [
  {x:  20, y: 340, w: BOX_W, h: BOX_H},
  {x: 285, y: 340, w: BOX_W, h: BOX_H},
  {x: 550, y: 340, w: BOX_W, h: BOX_H},
];
// R-pip positions — on box borders (integrated) and along connector segments
const R_POS = {
  // SOFTWARE bottom border (y=80), center x=101
  'R1.1':{x:  72, y: 80}, 'R1.2':{x: 101, y: 80}, 'R1.3':{x: 130, y: 80},
  // CONTROLLER bottom border (y=80), center x=267
  'R2.2':{x: 244, y: 80}, 'R2.3':{x: 290, y: 80},
  // RELAY bottom border (y=80), center x=433
  'R3.1':{x: 433, y: 80},
  // PUMP bottom border (y=80), center x=599
  'R4.1':{x: 575, y: 80}, 'R4.2':{x: 623, y: 80},
  // Main water vertical x=599, y 80→170
  'R5.1':{x: 599, y: 108}, 'R5.2':{x: 599, y: 145},
  // Control wire horizontal y=140, x 267→520
  'R6.1':{x: 330, y: 140}, 'R6.2':{x: 400, y: 140}, 'R6.3':{x: 470, y: 140},
  // VALVES bottom border y=230
  'R7.1':{x: 560, y: 230}, 'R7.2':{x: 586, y: 230},
  'R7.3':{x: 612, y: 230}, 'R7.4':{x: 638, y: 230},
  // Lateral hoses
  'R8.1':{x: 300, y: 290}, 'R8.2':{x: 638, y: 265},
  // SPRK2 bottom border (y=400)
  'R9.1':{x: 370, y: 400},
};
  ...
  return (
    <svg viewBox="0 0 700 415" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg">
      ...
      <style>{`
        ...
        .pip{font-family:'JetBrains Mono',monospace;font-weight:700;font-size:8px;pointer-events:none}
      `}</style>

      {/* ── control wires (rust dashed) ── */}
      <g fill="none" stroke="#b14a26" strokeWidth="1.5" strokeDasharray="6 3" strokeLinecap="round">
        {/* SW → CTRL */}
        <line x1="166" y1="50" x2="202" y2="50" markerEnd="url(#arrR)"/>
        {/* CTRL → RELAY */}
        <line x1="332" y1="50" x2="368" y2="50" markerEnd="url(#arrR)"/>
        {/* CTRL → VALVES: down from CTRL bottom, right across (R6 pips), down, into VALVES left edge */}
        <path d="M 267 80 V 140 H 520 V 200 H 534" markerEnd="url(#arrR)"/>
      </g>
      ...
      <g stroke="#1a2238" strokeWidth="2.2" fill="none" strokeLinecap="round">
        {/* to SPRK1 (center 85) */}
        <path d="M 560 230 V 290 H 85 V 340" markerEnd="url(#arr)"/>
        {/* to SPRK2 (center 350) */}
        <path d="M 599 230 V 310 H 350 V 340" markerEnd="url(#arr)"/>
        {/* to SPRK3 (center 615) */}
        <path d="M 638 230 V 290 H 615 V 340" markerEnd="url(#arr)"/>
      </g>

      {/* ── nodes (box + icon + label) ── */}
      {BOXES.map(b => {
        const cx = b.x + b.w/2;
        return (
          <g key={b.key}>
            <rect x={b.x} y={b.y} width={b.w} height={b.h} rx="3" fill="#f7f2e6" stroke="#1a2238" strokeWidth="1.5"/>
            <NodeIcon kind={b.icon} cx={cx} cy={b.y + 22}/>
            <text x={cx} y={b.y + 50} textAnchor="middle" className="lbl">{b.label}</text>
          </g>
        );
      })}
      {SPRK.map((s,i) => {
        ...
        return (
          <g key={`sp${i}`}>
            <rect x={s.x} y={s.y} width={s.w} height={s.h} rx="3" fill="#f7f2e6" stroke="#1a2238" strokeWidth="1.5"/>
            <NodeIcon kind="sprk" cx={cx} cy={s.y + 22}/>
            <text x={cx} y={s.y + 50} textAnchor="middle" className="lbl">SPRINKLER</text>
          </g>
        );
      })}

      {/* ── R-squares (integrated badges on borders / connectors) ── */}
      {ALL_IDS.map(rcId => {
