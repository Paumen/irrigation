...
The heads are the parts that actually spray water at the lawn. Three components (see `system.yaml` for what is fitted per zone):
...
- `system.yaml` — per-zone head counts and types for this system (`flow:`).
