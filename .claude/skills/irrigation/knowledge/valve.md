...
Where a manual hose is fed off the manifold (see `system.yaml`), the pump is started from the
...
landing in their 1.7–4.8 bar window (see `heads.md`, and `system.yaml` for what is actually fitted).
...
> Check the arrow first. (See `system.yaml`'s `equipment:` for install dates.)
...
identification (check `system.yaml` for what's installed).
...
system's per-zone flows are solved from `system.yaml`'s topology and `system.yaml`'s charts.)
