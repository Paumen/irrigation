...
- Vocabulary — use the left term: pump (not engine), irrigation system (not sprinkler system), heads/rotors (not sprinklers), well (not source), power supply (not mains), manual valve / hose (not ball valve / garden hose), hoses (not tubes / laterals / pipes), app (not software), metres / litres / bar (not feet / gallons / psi), arc (not angle), multimeter (not meter), splice (not wire connector), PTFE tape (not teflon tape). Full glossary + the rest of the canonical list in `references/terminology.md`.
