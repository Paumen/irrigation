...
Common shapes:
- **How many zones at once?** One. Settings run zones sequentially and the no-flow-sensor / no-master-valve design assumes it. Check each zone's computed flow (the tool's per-zone `flow_m3h` — zone flows aren't stored in `setup.yaml`, they're solved) sits under the pump's 3.8 m³/h.
