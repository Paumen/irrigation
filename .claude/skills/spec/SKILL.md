...
- **Put a cheap partial out early.** A stub, mockup, svg, png — the lowest-cost thing the user can react to — and let their reaction steer. Default to showing inline, not committing. SVG is the cheapest to author; render it to PNG to show with a standard converter (`pip install cairosvg`, e.g. `cairosvg in.svg -o out.png`, or a repo's `svg2png.py`). If a set of partials is kept around, name each one for the requirement(s) it addresses and what it is (e.g. `U6-diagram`) — one mockup per concern, no two competing schemes.
- **Show the PNG and stop — feedback gates everything else.** When the work produces a visual partial, rendering it and showing the PNG to the user is a hard first step: send the PNG, then *wait for the user's reaction* before taking any other step. Do not commit, push, open a PR, build variants, or move on until they've responded. The whole point is to be cheap to redirect — committing or branching ahead of feedback is exactly the rework this skill exists to avoid.
